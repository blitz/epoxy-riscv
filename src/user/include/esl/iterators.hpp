...
namespace esl
{
template <typename predicate_type, typename range_type>
class filter_range
{
...
public:
  ...
  public:
    auto operator*() { return *source_it; }
    ...
    bool operator==(filtered_it const &other) const { return source_it == other.source_it; }

    bool operator!=(filtered_it const &other) const { return source_it != other.source_it; }

    filtered_it(predicate_type const &predicate_,
                range_it const &source_it_,
                range_it const &end_it_)
  ...
  filtered_it begin() { return {predicate, range.begin(), range.end()}; }

  filtered_it end() { return {predicate, range.end(), range.end()}; }
...
template <typename transform_type, typename range_type>
...
public:
  ...
  public:
    auto operator*() { return transform(*source_it); }
    ...
    bool operator==(transformed_it const &other) const { return source_it == other.source_it; }
  ...
  transformed_it begin() { return {transform, range.begin()}; }

  transformed_it end() { return {transform, range.end()}; }
